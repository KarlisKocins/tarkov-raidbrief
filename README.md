...
TarkovMonitor) with task data from the **tarkov.dev** GraphQL API, and gives you
a per-map brief: what to **carry in**, which **keys** you need, what to **bring
out**, and what to actually **do**.
...
**This add-on uses relative URLs throughout the frontend.** Every `href`, `src`
and `fetch()` in the page is relative (`static/app.css`, `fetch("api/refresh")`),
so they resolve correctly under the ingress prefix *and* on the direct port with
no server-side rewriting.

As a secondary measure, a middleware also reads `X-Ingress-Path` and sets
Starlette's `root_path` per request, so anything server-generated (`url_for`,
the `/docs` OpenAPI page) lands on the right prefix too. The relative URLs are
the load-bearing mechanism; `root_path` is belt and braces.
...
- The tarkov.dev task dump is cached to `/data/tasks.json` with a **24h TTL** —
  it's a large response and only changes on patch day. It survives add-on
  restarts. If tarkov.dev is unreachable the cached copy is served with a
...
The tarkov.dev schema moves between patches, so the GraphQL query is assembled
from named optional blocks (`requiredKeys`, `zones`, `shootDetail`,
`questItemLocations`). If the full query is rejected, the add-on drops one block
at a time and retries, logging exactly which block went. Worst case you lose the
KEYS section; you never lose the app.
...
└── tarkov_raidbrief/               # the add-on itself
    ...
    └── app/
        ...
        ├── tarkovdev.py  # GraphQL client, composable query, disk cache
